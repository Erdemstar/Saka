...
namespace cross_site_request_forgery_weak_token.Mvc.Controllers;
...
public class HomeController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> Attack([FromQuery] string status, string csrfToken)
    {
        ...
        if (string.IsNullOrEmpty(status) || string.IsNullOrEmpty(csrfToken))
        {
            if (accountEntity is null)
            {
        ...
        if (string.IsNullOrEmpty(status) || status != "deleted")
        {
            ViewBag.AttackError = "There is a problem while getting current user account information";
            return View(accountEntity);
        }
        ...
        var csrfTokenValue = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(csrfToken));

        if (csrfTokenValue != DateTime.Now.ToString("dd/MM/yyyy"))
        {
            ViewBag.AttackError = "Invalid CSRF token";
            return View(accountEntity);
        }
